...
function getData(url){
	if(url=="") url = "expectedJson.js";
	var firstName = "";
	var lastName = "";

	$.getJSON(url, function(data){
		console.log(data);
		$("#firstName").html("Welcome : " + data.firstName + " " + data.lastName);
...
microphone.onwebkitspeechchange = function(e){
	...
	$(responses).each(function(){
		...
		if(this.utterance.indexOf("Ford") > -1){
			...
			$("#inputBox").val(this.utterance);
			$("#output").html("Calculating..<img style='height: 30px; width: 30px' src='/resources/images/loading.gif'/>");
			
			setTimeout(function(){
				playSpeech("That price will be $52 per month");
				$("#output").html("That price will be $52 per month");
			}, 1000);
		}
